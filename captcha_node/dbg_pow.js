...
<div id="cap-holder"></div><button id="cap-btn" type="button"></button>
...
const dom = new JSDOM(html, {
  ...
  beforeParse(window) {
    ...
    // Perf timing patch: high-res timing with jitter (common POW/fingerprint check)
    const origNow = performance.now.bind(performance);
    window.performance.now = () => origNow() + Math.random() * 0.1;
  },
});
const { window } = dom;
...
(async () => {
  try { await waitFor(() => typeof window.initAliyunCaptcha === 'function'); } catch { console.log('SDK NEVER LOADED'); process.exit(3); }
  ...
  window.initAliyunCaptcha({
    ...
    getInstance: (inst) => { try { if (inst.startTracelessVerification) inst.startTracelessVerification(); } catch (e) { console.error('start err', e.message); } },
  ...
  setTimeout(() => { console.log('GIVEUP'); process.exit(2); }, 30000);
